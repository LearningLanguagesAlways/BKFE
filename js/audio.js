...
(function () {
  ...
  function loadVoices() {
    ...
    state.voices = all.filter(function (v) {
      return (v.lang || "").toLowerCase().indexOf("ko") === 0;
    });
    // Prefer a Korean voice; remember user's saved choice if any
    var saved = null;
    ...
    bar.innerHTML =
      ...
      '<div class="tts-control__panel" hidden>' +
        ...
        '<label class="tts-control__row">속도 · Speed' +
          '<input type="range" min="0.5" max="1.1" step="0.1" value="' + state.rate + '" class="tts-rate">' +
